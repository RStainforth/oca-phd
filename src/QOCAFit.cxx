...
Float_t QOCAFit::Model(Int_t irun, Int_t jpmt, Int_t na,Float_t *dyda)
{
  ...
  if (derivatives) {
    ...
    if(!fLBdistType) dyda[Getlbdistpari()+filbdist] = +1.0/laserlight;
    else {
  ...
  if(fOccType == 1) {
	...
	if(!fLBdistType) dyda[Getlbdistpari()+fcilbdist] -= 1.0/laserlight;
	else {
	  Double_t* parlb = new Double_t[1+fNdistwave];
	  Double_t* lbphi = new Double_t(phi);
...
//		if (dyda[fparindex[i]] == 0.0) {
//			printf("Model: null derivative i %d parindex %d\n",i,fparindex[i]);
//			printf("LB par %d angresp %d laserlight %f\n",
//			Getlbdistpari()+filbdist,Getangresppari()+fiang,laserlight);
//		}
    }
