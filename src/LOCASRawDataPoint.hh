...
///        to obtain a data set ready for use in a fit (see LOCASDataFiller)
///        
///          
...
namespace LOCAS{
  ...
  public: 
    ...
    // Constructors
    LOCASRawDataPoint(){ }
    LOCASRawDataPoint( const LOCASPMT* pmt, const LOCASRun* run );
    ...
    void SetPMTPos( const TVector3& val ){ fPMTPos = val; }

    void SetLBPos( const TVector3& val ){ fLBPos = val; }
  ...
  private:
    ...
    Float_t fTotalDist;                  // Total Light Path Distance
    Float_t fCentralTotalDist;           // Central Total Light Path Distance
