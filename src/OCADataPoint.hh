////////////////////////////////////////////////////////////////////
///
/// FILENAME: OCADataPoint.hh
///
/// CLASS: OCA::OCADataPoint
///
...
///
/// DETAIL: OCADataPoints are used by a OCAChiSquare object 
///         which, combined with a OCAOptics model to perform a fit
...
#ifndef _OCADataPoint_
#define _OCADataPoint_
...
namespace OCA{
  ...
  class OCADataPoint : public TObject
  {
  public:

    // The constructors and destructors for the OCADataPoint object.
    OCADataPoint(){ };
    OCADataPoint( const OCAPMT& lPMT );
    ~OCADataPoint(){ };
    
    // Equality operator
    OCADataPoint& operator=( const OCADataPoint& rhs );
    ...
    // a OCADataStore object. The index starts at 0.
    ...
    // only initialised once a OCAChiSquare object has computed the 
    // chi-square value for this data point.
    Float_t GetModelOccupancyRatio() const { return fModelOccupancyRatio; }

    // Get the value to of the raw efficiency as calculated by 'oca2fit2eff'.
    Float_t GetRawEfficiency() const { return fRawEfficiency; }

    // Get the value to of the normalised efficiency as calculated by 'oca2fit2eff'.
    Float_t GetNormalisedEfficiency() const { return fNormalisedEfficiency; }

    // Get the value to of the PMT variability as calculated by 'oca2fit2eff'.
    ...
    // Set the value to of the raw efficiency as calculated by 'oca2fit2eff'.
    void SetRawEfficiency( const Float_t val ){ fRawEfficiency = val; }

    // Set the value to of the normalised efficiency as calculated by 'oca2fit2eff'.
    void SetNormalisedEfficiency( const Float_t val ){ fNormalisedEfficiency = val; }

    // Set the value to of the PMT variability as calculated by 'oca2fit2eff'.
  ...
  private:
    ...
    Float_t fLBIntensityNorm;               // The run intensity normalisation value for the off-axis run (calculated by OCARun::CalculateLBIntensityNorm)
    Float_t fCentralLBIntensityNorm;        // The run intensity normalisation value for the central run (calculated by OCARun::CalculateLBIntensityNorm)
    ...
    Float_t fRawEfficiency;                 // The estimate for the raw efficieny
                                            // of this data point. ( fMPECorrOccupancy / OCAOpticsModel::ModelPrediction [after minimisation] ).

    Float_t fNormalisedEfficiency;          // The estimate for the normalised efficieny.
    Float_t fPMTVariability;                // The PMT variability as calculated by 'oca2fit2eff'.   

    ClassDef( OCADataPoint, 1 );
