...
void OCAFit::LoadFitFile( const char* fitFile )
{
  ...
  // OCARunReader object (OCA version of TChain)
  ...
  fPMTPtrs = new OCAPMT*[ ( fNRuns * 10000 ) ];

  // Set up the pointers to the individual runs
  fRunPtrs = new OCARun*[ fNRuns ];
  for (Int_t iRun = 0; iRun < fNRuns; iRun++ ){
    fRunPtrs[ iRun ] = new OCARun();
    *fRunPtrs[ iRun ] = *( fRunReader.GetRunEntry( iRun ) );
    fRunPtrs[ iRun ]->CopyOCAPMTInfo( *( fRunReader.GetRunEntry( iRun ) ) );
    for ( fiPMT = fRunPtrs[ iRun ]->GetOCAPMTIterBegin();
          fiPMT != fRunPtrs[ iRun ]->GetOCAPMTIterEnd();
          fiPMT++ ){
      fPMTPtrs[ (iRun*10000) + ( ( fiPMT->first ) % 10000 ) ] = new OCAPMT(fRunPtrs[ iRun ]->GetPMT( fiPMT->first ));
    } 
  }  
  ...
  fMrqX = OCAMath::OCAVector( 1, fNDataPointsInFit );
  fMrqY = OCAMath::OCAVector( 1, fNDataPointsInFit );
  fMrqSigma = OCAMath::OCAVector( 1, fNDataPointsInFit );
  ...
  // each PMT is cut on the below criteria in OCAFit::PMTSkip and OCAFit::DataScreen.
...
void OCAFit::InitialiseParameters()
{
  ...
  for ( Int_t iT = 0; iT < fNRuns; iT++ ){
    ...
    for ( fiPMT = fCurrentRun->GetOCAPMTIterBegin();
...
void OCAFit::AllocateParameters( )
{
  ...
  fMrqParameters = OCAMath::OCAVector( 1, fNParametersInFit );
  fMrqVary = OCAMath::OCAIntVector( 1, fNParametersInFit );

  fMrqAlpha = OCAMath::OCAMatrix( 1, fNParametersInFit, 1, fNParametersInFit );
  fMrqCovariance = OCAMath::OCAMatrix( 1, fNParametersInFit, 1, fNParametersInFit );
...
void OCAFit::PrintInitialisationInfo( )
{
 
  cout << "OCAFit Parameters Initialised" << endl;
...
void OCAFit::DataScreen( const Float_t chiSqLimit )
{
  ...
  for ( Int_t iRun = 0; iRun < fNRuns; iRun++ ){
    ...
    // for ( fiPMT = fCurrentRun->GetOCAPMTIterBegin(); fiPMT != fCurrentRun->GetOCAPMTIterEnd(); fiPMT++ ){
    ...
    for ( fiPMT = fCurrentRun->GetOCAPMTIterBegin(); 
      ...
      if( !skipPMT ){
        ...
        // Note: Not including PMT variability sigma in here, this was in OCA (SNO)
  ...
  for ( Int_t iBin = 0; iBin < nLBDistBins; iBin++ ){ 
    ...
    printf( "Laserball distribution bin %d, itheta %d iphi %d has %d PMT's\n",
            iBin,
  ...
  // in a std::map< Int_t, OCAPMT > map (fFitPMTs)
  Int_t oldTmpRun2 = 0;
  for ( Int_t iPM = 1; iPM <= fNPMTsInFit; iPM++ ){
    ...
    OCAPMT lPMT = fCurrentRun->GetPMT( pmtID );
    fFitPMTs[ iX ] = lPMT;
...
Bool_t OCAFit::PMTSkip( const OCARun* iRunPtr, const OCAPMT* iPMTPtr, Float_t mean, Float_t sigma )
{
  ...
  else if ( iPMTPtr->GetBadPath() || occCount < 100.0 ){ 
    if ( !pmtSkip ){ 
      pmtSkip = true; 
      fSkipBad++; 
    }
...
TH1F* OCAFit::AngularResponseTH1F()
{
...
Double_t OCAFit::SPMTAngularResponse( Double_t* a, Double_t* par )
{
  ...
  // see OCAFit::ModelAngularResponse() when modifying...
...
TF1* OCAFit::AngularResponseTF1()
{

  // Returns a pointer to a function with parameters equal to angular response
  // parameters from OCAFit, to plot the current PMT angular response function.
...
Float_t OCAFit::ModelAngularResponse( const OCAPMT* iPMTPtr, Int_t& iAng, Int_t runType )
{
  ...
  Float_t angularResponse = GetAngularResponsePar( iAng );
  if ( std::isnan( angularResponse ) ){
    //cout << "AngularResponse is nan, iAng is: " << iAng << endl;
    //cout << "Parameter index is: " << GetAngularResponseParIndex() + iAng << endl;
    //cout << "fMrParameters Value is: " << fMrqParameters[ GetAngularResponseParIndex() + iAng ] << endl;
  }
...
Float_t OCAFit::ModelLBDistribution( const OCARun* iRunPtr, const OCAPMT* iPMTPtr, Int_t& iLBDist, Int_t runType )
{
...
Float_t OCAFit::ModelLBDistribution( const Float_t cosTheta, Float_t phi, Int_t& iLBDist )
{
...
Float_t OCAFit::ModelLBDistributionMask( const OCAPMT* iPMTPtr, const Int_t runType )
{
...
Float_t OCAFit::ModelLBDistributionMask( const Float_t cosTheta )
{
...
TF1* OCAFit::LBDistributionMaskTF1()
{
  ...
  // OCAFit::GetNLBDistributionMaskParameters() for the number of parameters.
...
TF1* OCAFit::LBDistributionWaveTF1( const Int_t iTheta )
{
  ...
  if( iTheta < 0 
      || iTheta >= fNLBDistributionThetaWaveBins ) { printf("OCAFit::LBDistributionWaveTF1: The theta slice %d does not exist.",iTheta); }
...
TH2F* OCAFit::ApplyLBDistribution()
{
...
Float_t OCAFit::SLBDistributionMask( Double_t* aPtr, Double_t* parPtr )
{
...
Float_t OCAFit::DLBDistributionMask( Double_t* aPtr, Double_t* parPtr )
{
...
Float_t OCAFit::SLBDistributionWave( Double_t* aPtr, Double_t* parPtr )
{
...
Float_t OCAFit::DLBDistributionWave( Double_t* aPtr, Double_t* parPtr )
{
...
Float_t OCAFit::CalculatePMTChiSquare( const OCARun* iRunPtr, const OCAPMT* iPMTPtr )
{
...
Float_t OCAFit::CalculateChiSquare()
{
  ...
  if ( !fDataScreen ){ cout << "OCAFit::CalculateChiSquare: Error, run OCAFit::DataScreen first"; }
  else{
...
Float_t OCAFit::CalculatePMTVariability( const OCAPMT* iPMTPtr )
{
...
Float_t OCAFit::CalculatePMTSigma( const OCAPMT* iPMTPtr )
{
...
Float_t OCAFit::CalculatePMTData( const OCAPMT* iPMTPtr )
{
...
Float_t OCAFit::ModelPrediction( const OCARun* iRunPtr, const OCAPMT* iPMTPtr, Int_t nA, Float_t* dyda )
{
...
void OCAFit::PerformFit()
{

  if ( fDataScreen == true && fFitPMTs.size() > 0 ){

    fChiSquare = 0.0;
    cout << "OCAFit::PerformFit: Performing fit...\n";
    FillParameterbase();
    FillAngIndex();
...
void OCAFit::FillParameterPoint()
{
...
void OCAFit::WriteFitToFile( const char* fileName )
{

  TFile* file = TFile::Open( fileName, "RECREATE" );;

  TTree* runTree = new TTree( "OCAFitT", "OCAFit" );
...
void OCAFit::DeAllocate()
{
  
  if ( fCentralRunNorms ){ delete fCentralRunNorms; }

  // Free up all the memory used during the fitting procedure.
  // This method is called by the OCAFit desctructor.

  if ( fMrqX ){ OCAMath::OCAFree_Vector( fMrqX, 1 ); }
  fMrqX = NULL;

  if ( fMrqY ){ OCAMath::OCAFree_Vector( fMrqY, 1 ); }
  fMrqY = NULL;

  if ( fMrqSigma ){ OCAMath::OCAFree_Vector( fMrqSigma, 1 ); }
  fMrqSigma = NULL;
  ...
  if ( fMrqParameters ){ OCAMath::OCAFree_Vector( fMrqParameters, 1 ); }
  fMrqParameters = NULL;

  if ( fMrqVary ){ OCAMath::OCAFree_IntVector( fMrqVary, 1 ); }
  fMrqVary = NULL;

  if ( fMrqCovariance ){ OCAMath::OCAFree_Matrix( fMrqCovariance, 1, 1 ); }
  fMrqCovariance = NULL;

  if ( fMrqAlpha ){ OCAMath::OCAFree_Matrix( fMrqAlpha, 1, 1 ); }
  fMrqAlpha = NULL;
...
void OCAFit::PrintCovarianceMatrix(){
...
void OCAFit::PrintParameterInformation(){
...
Int_t OCAFit::MrqFit( float x[], float y[], float sig[], int ndata, float a[],
...
Int_t OCAFit::mrqmin(float x[], float y[], float sig[], int ndata, float a[], 
                       int ia[], int ma, float **covar, float **alpha, float *chisq, 
                       float *alamda)
{
  ...
  //	void (*funcs)(float, float [], float *, float [], int));
	...
	if (*alamda < 0.0) {
      atry=OCAMath::OCAVector(1,ma);
      beta=OCAMath::OCAVector(1,ma);
      da=OCAMath::OCAVector(1,ma);
      for (mfit=0,j=1;j<=ma;j++){
        if (ia[j]){
          mfit++;
        }
      }
      oneda=OCAMath::OCAMatrix(1,mfit,1,1);
      *alamda=0.001;
	...
	if (*alamda == 0.0 ) {
      ...
      //printf("POST_COVSRT\n");
      //PrintCovarianceMatrix();
      OCAMath::OCAFree_Matrix(oneda,1,1);
      OCAMath::OCAFree_Vector(da,1);
      OCAMath::OCAFree_Vector(beta,1);
      OCAMath::OCAFree_Vector(atry,1);
      return retval;
	}
	for (j=0,l=1;l<=ma;l++)
...
void OCAFit::covsrt( float **covar, int ma, 
                      int ia[], int mfit )
{
...
void OCAFit::mrqcof(float x[], float y[], float sig[], int ndata, float a[],
    			...
    			float *chisq)
{
  ...
  float *beta2 = OCAMath::OCAVector(1,ma);
  float **alpha2 = OCAMath::OCAMatrix(1,ma,1,ma);
  dyda=OCAMath::OCAVector(1,ma);
  for (j=1;j<=ma;j++) {
    if (ia[j]) mfit++;
    dyda[j] = 0.0;
  }
  ...
  *chisq=0.0;
  ...
  OCAMath::OCAFree_Vector(dyda,1);
  
  OCAMath::OCAFree_Matrix(alpha2,1,1);
  OCAMath::OCAFree_Vector(beta2,1);

}
//////////////////////////////////////
//////////////////////////////////////

void OCAFit::mrqfuncs(Float_t x,Int_t ix,Float_t a[],Float_t *y,
...
Int_t OCAFit::gaussj(float **a, int n, float **b, int m)
{
...
//
	...
	Int_t retval = 0;
	indxc=OCAMath::OCAIntVector(1,n);
	indxr=OCAMath::OCAIntVector(1,n);
	ipiv=OCAMath::OCAIntVector(1,n);
	...
	OCAMath::OCAFree_IntVector(ipiv,1);
	OCAMath::OCAFree_IntVector(indxr,1);
	OCAMath::OCAFree_IntVector(indxc,1);
	return retval;
...
Int_t OCAFit::GetNVariableParameters()
{
...
void OCAFit::SetAngularResponseVary( Bool_t varyBool )
{
...
void OCAFit::SetAngularResponse2Vary( Bool_t varyBool )
{
...
void OCAFit::SetLBDistributionMaskVary( Bool_t varyBool )
{
...
void OCAFit::SetLBDistributionVary( Bool_t varyBool )
{
...
void OCAFit::SetLBDistributionWaveVary( Bool_t varyBool )
{
  Int_t boolInt = 0;
  if ( varyBool == true ){ boolInt = 1; }
  else{ boolInt = 0; }
...
void OCAFit::SetLBNormalisationVary( Bool_t varyBool )
{
...
Int_t OCAFit::GetLBDistributionMaskParIndex()
{
...
Int_t OCAFit::GetLBDistributionWaveParIndex()
{
...
Int_t OCAFit::GetAngularResponseParIndex()
{
...
Int_t OCAFit::GetAngularResponse2ParIndex()
{
...
Int_t OCAFit::GetLBDistributionParIndex()
{
...
Int_t OCAFit::GetLBNormalisationParIndex()
{
...
Int_t OCAFit::GetRunIndex( const Int_t runID )
{
  ...
  for ( Int_t iG = 0; iG < fNRuns; iG++ ){
    ...
    if( iG == fNRuns - 1 ){
      cout << "OCAFit::GetRunIndex: Error: No Index found for given run ID:" << runID << endl;
      return 0;
    }
  }
  return 0;
...
Float_t OCAFit::GetAngularResponseError( const Int_t nVal )
{
  ...
  if ( nVal >= 0 && nVal < fNAngularResponseBins ) {
    ...
    printf( "OCAFit::GetAngularResponseError: (%d) is out of range [ 0:%d ]\n", nVal, fNAngularResponseBins - 1 );
    return 0;
  }
}
...
Float_t OCAFit::GetLBDistributionError( const Int_t nVal )
{
  ...
  if( fLBDistributionType == 0 ){
    ...
    if ( nVal >= 0 && nVal < fNLBDistributionPhiBins * fNLBDistributionThetaBins ) {
      ...
      printf("OCAFit::GetLBDistributionError( %d ) is out of range [ 0:%d ]\n", nVal, fNLBDistributionPhiBins * fNLBDistributionThetaBins - 1 );
      return 0;
    }
  }
  ...
  else { printf("OCAFit::GetLBDistributionError( %d ) is out of range [ 0:%d ]\n", nVal, fNLBDistributionPhiBins * fNLBDistributionThetaBins - 1 );
      return 0;
    }
  }
}
...
Float_t OCAFit::GetLBDistributionMaskError( const Int_t nVal )
{
  // Return the square root of the diagonal element corresponding to the
...
TGraph* OCAFit::PlotModelROccVsDataROcc()
{
  ...
  for ( Int_t iK = 1; iK <= fNPMTsInFit; iK++ ){
    ...
    Int_t runN = iX / 10000;
    fCurrentRun = fRunPtrs[ runN ];
    fCurrentPMT = fPMTPtrs[ (runN*10000) + pmtID ];
    plot->SetPoint( plotPoint++, ModelPrediction( fCurrentRun, fCurrentPMT ), fMrqY[ iK ] );
  }

  plot->GetXaxis()->SetTitle( "Model ROcc" );
  plot->GetYaxis()->SetTitle( "Data ROcc" );
