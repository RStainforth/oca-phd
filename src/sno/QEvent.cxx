...
Bool_t QEvent::IsHe4(){
  ...
  for(j=0;j<nMuxScopes;j++)if(GetMuxScope(j)->IsHe4())result=kTRUE;
  for(j=0;j<nPartialMuxScopes;j++)if(GetPartialMuxScope(j)->IsHe4())result=kTRUE;
  for(j=0;j<nShapers;j++)if(GetShaper(j)->IsHe4())result=kTRUE;
  return result;
}
