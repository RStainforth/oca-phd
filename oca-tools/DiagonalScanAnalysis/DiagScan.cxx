...
void DiagScan::Process(){
  ...
  /// The PMTs that are within a fixed distance to the diagonal will be selected and all  
  /// the necessary corrections to the occupancy are calculated. Pairs of PMTs (one from
  /// each group) are formed and their occupancy ratio and product are calculated (including
  /// their corresponding uncertainities).The weighted mean value of the ratio and product 
  /// for the run is calculated.
	
  cout << "Processing the Data!" << endl;

  // Diagonal selection
  TVector3 fDiagonalVec; // Vector representing the diagonal
  if( fScan == "xpz" )  fDiagonalVec = TVector3(1,0,1);
  if( fScan == "xnz" )  fDiagonalVec = TVector3(1,0,-1);
...
void DiagScan::FitRatio(){
	...
	sprintf(name1,"Ratio, Diagonal %s, wavelength %d nm",fScan,fLambda); 
	gr->SetTitle(name1);
...
//	cout << "getting coefficients" << endl;
	...
	c0->Update();
	sprintf(savename1,"FittedRatio%s%dTol%dSh%d.pdf",fScan,fLambda,(int)fTolerance,(int)fShadowing);
...
void DiagScan::Product(){
	...
	prod->Draw("AP");
	sprintf(name2,"Product, Diagonal %s, wavelength %d nm",fScan,fLambda); 
	prod->SetTitle(name2);
	...
	sprintf(savename2,"Product%s%dTol%dSh%d.pdf",fScan,fLambda,(int)fTolerance,(int)fShadowing);
